...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for toc in orm['data_manager.TOC'].objects.all():
            for theme in toc.themes.all():
                order = len(orm['data_manager.TOCThemeOrder'].objects.filter(toc=toc))+1
                orm['data_manager.TOCThemeOrder'].objects.get_or_create(order=order, toc=toc, theme=theme)
