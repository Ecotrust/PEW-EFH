    ...
    if username and password:
        try:
            ...
            if not user.is_active:
                return HttpResponse(simplejson.dumps({'verified': 'inactive'}), content_type="application/json", status=200)
            if user.check_password(password):
                return HttpResponse(simplejson.dumps({'verified': True}), content_type="application/json", status=200)
            else:
                return HttpResponse(simplejson.dumps({'verified': False}), content_type="application/json", status=200)
        except:
            return HttpResponse(simplejson.dumps({'verified': False}), content_type="application/json", status=200)
    else:
...
def duplicate_username(request):
    ...
    return HttpResponse(simplejson.dumps({'duplicate': found}), content_type="application/json", status=200)
